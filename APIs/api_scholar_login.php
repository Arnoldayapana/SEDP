...
switch ($method) {
    case 'GET':
        if (isset($_GET['id'])) {
            $id = intval($_GET['id']); // Use 'employee_id' here
            $stmt = $pdo->prepare("SELECT * FROM scholar_login WHERE id = ?");
            $stmt->execute([$id]);
            ...
            $stmt = $pdo->query("SELECT * FROM scholar_login");
    ...
    case 'POST':
        ...
        $stmt = $pdo->prepare("INSERT INTO scholar_login (username, password) VALUES (?, ?)");
        $result = $stmt->execute([$data->username, $data->password]);
    ...
    case 'PUT':
        $data = json_decode(file_get_contents("php://input"));
        $stmt = $pdo->prepare("UPDATE scholar_login SET username = ?, password = ? WHERE id = ?");
    ...
    case 'DELETE':
        if (isset($_GET['id'])) {
            $id = intval($_GET['id']);
            $stmt = $pdo->prepare("DELETE FROM scholar_login WHERE id = ?");
